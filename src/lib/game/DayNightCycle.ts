...
// Full cycle in seconds (real time). 4 minutes = fun pace for exploration.
const FULL_CYCLE_DURATION = 240;

export class DayNightCycle {
  private overlay: THREE.Mesh;
  private scene: THREE.Scene;
  private time: number = 0; // 0-1 normalized
  ...
  constructor(scene: THREE.Scene) {
    ...
    this.overlay.position.z = 3.5; // above weather overlay
  ...
  update(deltaTime: number, playerX: number, playerY: number) {
    ...
    // Phases: dawn 0.0-0.15, day 0.15-0.55, dusk 0.55-0.7, night 0.7-1.0
    let phase: TimeOfDay;
    let blend = 0; // 0-1 within phase
    if (this.time < 0.15) {
    ...
    // Edge transitions (first/last 20% of each phase)
    ...
    const bgA = new THREE.Color(cfg.bgColor);
    const bgB = new THREE.Color(nextCfg.bgColor);
    const transBlend = blend > 0.8 ? (blend - 0.8) / 0.2 : 0;
    this.sceneBackground.copy(bgA).lerp(bgB, transBlend);
